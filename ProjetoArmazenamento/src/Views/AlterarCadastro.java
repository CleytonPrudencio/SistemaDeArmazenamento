...
public class AlterarCadastro extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AlterarCadastro frame = new AlterarCadastro();
	...
	public AlterarCadastro() {
		...
		btngravar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				conex.Conexao();
				conex.executaSql("select id from usuarios where usuario like '" + newuser + "'");
				String id = null;
				try {
					while (conex.rs.next()) {
					id = conex.rs.getString("id");
						//pegarid.setID(conex.rs.getString("id"));//Faz a contagem dos chamados
						//JOptionPane.showMessageDialog(null, "Entrou aqui no ID" + id);
					}
						...
						if (newuser.equals(null) || newuser.equals("") || senhaantiga.equals(null) || senhaantiga.equals("")) {
							...
							//JOptionPane.showMessageDialog(null, "Entrou aqui" + id + newuser + nome);
							conex.Conexao();
							conex.executaSqlUpdate("UPDATE `armazenamento_sistema`.`usuarios` SET `usuario` = '" + newuser + "', `senha` = '" + senha + "', `nome_completo` = '" + nome + "', `Cargo` = '" + cargo + "' where `id` = '" + id + "';");
